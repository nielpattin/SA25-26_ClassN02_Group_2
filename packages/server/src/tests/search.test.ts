...
describe('Search API - @assignee operator', () => {
  ...
  let taskId2: string
  let taskId3: string
  const secondUserId = '00000000-0000-4000-a000-000000000002'
  ...
  beforeAll(async () => {
    ...
    await db.update(schema.users)
      .set({ name: 'johndoe' })
      .where(eq(schema.users.id, secondUserId))

    const wsRes = await app.handle(
      new Request('http://localhost/v1/workspaces', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'Search Test Workspace', slug: `search-test-${Date.now()}` })
      })
    )
    ...
    const boardRes = await app.handle(
      new Request('http://localhost/v1/boards', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'Search Test Board', workspaceId })
      })
    )
    ...
    const columnRes = await app.handle(
      new Request('http://localhost/v1/columns', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'To Do', boardId, position: 'a0' })
      })
    )
    const column = await columnRes.json() as { id: string }
    columnId = column.id
  ...
  beforeAll(async () => {
    ...
    const wsRes = await app.handle(
      new Request('http://localhost/v1/workspaces', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'Label Search Workspace', slug: `label-search-${Date.now()}` })
      })
    )
    const ws = await wsRes.json() as { id: string }
    workspaceId = ws.id
    ...
    const boardRes = await app.handle(
      new Request('http://localhost/v1/boards', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'Label Search Board', workspaceId })
      })
    )
    const board = await boardRes.json() as { id: string }
    boardId = board.id
  ...
  beforeAll(async () => {
    ...
    const wsRes = await app.handle(
      new Request('http://localhost/v1/workspaces', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'Due Search Workspace', slug: `due-search-${Date.now()}` })
      })
    )
    const ws = await wsRes.json() as { id: string }
    workspaceId = ws.id
    ...
    const boardRes = await app.handle(
      new Request('http://localhost/v1/boards', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ name: 'Due Search Board', workspaceId })
      })
    )
    const board = await boardRes.json() as { id: string }
    boardId = board.id
