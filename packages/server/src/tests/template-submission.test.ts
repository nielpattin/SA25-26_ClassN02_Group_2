...
describe('Template Submission & Review', () => {
  ...
  test('POST /v1/templates/marketplace/:id/approve - admin approves template', async () => {
    ...
    expect(template.approvedBy).toBe(TEST_USER_ID)
  })
  ...
  test('Permissions - non-admin cannot approve', async () => {
     // We'd need another user for this, but our helpers use TEST_USER_ID
     // I'll skip this for now or use a dummy ID if possible
  })
})
