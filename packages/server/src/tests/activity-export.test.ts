...
import { db } from '../db'
import { workspaces, users, boards, boardMembers, activities, columns, tasks } from '../db/schema'
...
describe('Activity Export', () => {
  ...
  beforeAll(async () => {
    ...
    await db.delete(users).where(eq(users.id, 'activity-export-user'))
    await db.delete(users).where(eq(users.id, 'activity-export-admin'))

    // Setup admin user
    const [admin] = await db.insert(users).values({
      id: 'activity-export-admin',
      name: 'Activity Export Admin',
    ...
    const [user] = await db.insert(users).values({
      id: 'activity-export-user',
      name: 'Activity Export User',
    ...
    // Add regular user as member (non-admin)
    ...
    // Create column and task for activities
  ...
  describe('Repository', () => {
    it('should find activities by board ID in date range', async () => {
      ...
      const activities = await activityRepository.findByBoardIdInRange(
        testBoard.id,
        threeDaysAgo,
        now
      )
      ...
      expect(activities[1].action).toBe('updated')
      ...
      expect(activities[1].createdAt <= activities[2].createdAt).toBe(true)
    })
  ...
  describe('Service', () => {
    ...
    it('should throw BadRequestError for invalid date range', async () => {
      const now = new Date()
      const threeDaysAgo = new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000)
      const dateFrom = now.toISOString().split('T')[0] // From is after To
  ...
  describe('API Endpoints', () => {
    // Helper to parse streaming response from Elysia test handler
    const parseStreamResponse = async (res: Response): Promise<string> => {
      const content = await res.text()
      // Handle case where stream is serialized as multiple JSON objects
      // Each chunk is a Uint8Array serialized as {"0": byte0, "1": byte1, ...}
      const chunks: number[] = []
    ...
    it('GET /v1/activities/board/:id/export should return 200 for board admin', async () => {
      ...
      const res = await app.handle(
        new Request(
          ...
          {
            headers: { 'x-test-user-id': testAdmin.id }
          }
        )
      )
