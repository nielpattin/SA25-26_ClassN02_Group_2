...
export function Popover({ isOpen, onClose, children, triggerRef, title, matchTriggerWidth }: PopoverProps) {
  ...
  return createPortal(
    <div 
      className="overscroll-behavior-contain z-20000 flex min-w-70 flex-col rounded-none border border-black bg-white shadow-brutal-md transition-opacity duration-100" 
      ref={popoverRef} 
      style={{ 
        top: `${coords.top}px`, 
        left: `${coords.left}px`,
    ...
    >
      {title && (
        <div className="flex shrink-0 items-center justify-between border-b border-black bg-white px-4 py-3">
          ...
          <button className="cursor-pointer border-none bg-transparent p-1 text-xl leading-none font-extrabold text-black hover:text-accent" onClick={onClose}>&times;</button>
        </div>
      )}
