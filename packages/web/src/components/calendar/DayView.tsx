    ...
    <div className="shadow-brutal-lg flex h-full flex-col border border-black bg-white">
      <div className={`flex items-center justify-between border-b border-black p-4 ${isCurrentToday ? 'bg-accent' : 'bg-accent/10'}`}>
        ...
        <div className="flex items-center gap-2">
          <span className="shadow-brutal-sm border border-black bg-white px-3 py-1 text-sm font-black uppercase">
            {dayTasks.length} {dayTasks.length === 1 ? 'Task' : 'Tasks'}
          </span>
          <button
            onClick={() => setIsCreating(true)}
            className="hover:bg-accent shadow-brutal-sm border border-black bg-white px-3 py-1 text-sm font-black uppercase transition-all hover:-translate-x-px hover:-translate-y-px hover:shadow-none"
          >
            + Add Task
          </button>
        </div>
      ...
      <div 
        className="bg-canvas/50 flex-1 overflow-y-auto p-6"
        onClick={() => {
          if (columns.length > 0) {
            setIsCreating(true)
          }
        ...
        <div className="mx-auto flex max-w-2xl flex-col gap-3">
          {dayTasks.length === 0 && !isCreating ? (
            <div
              ...
              className="shadow-brutal-sm flex flex-col items-center justify-center border-2 border-dashed border-black/20 bg-white py-12 text-center"
            >
              ...
              {isCreating && (
                <div className="mt-1" onClick={e => e.stopPropagation()}>
                  <input
                    ref={inputRef}
                    type="text"
                    className="shadow-brutal-md w-full border-2 border-black bg-white p-3 text-sm font-bold outline-none"
