...
export const Route = createFileRoute('/templates/marketplace')({
  ...
  validateSearch: (search: Record<string, unknown>): { q?: string; category?: string; sort?: 'newest' | 'popular' | 'alphabetical'; page?: number } => {
    return {
      q: (search.q as string) || undefined,
      category: (search.category as string) || undefined,
      sort: (search.sort as 'newest' | 'popular' | 'alphabetical') || undefined,
...
type MarketplaceSearch = { q?: string; category?: string; sort?: 'newest' | 'popular' | 'alphabetical'; page?: number }
  ...
  const updateSearch = (newParams: Partial<MarketplaceSearch>) => {
    navigate({
      search: (prev: MarketplaceSearch) => ({
        ...
        page: newParams.q !== undefined || newParams.category !== undefined || newParams.sort !== undefined 
          ? undefined 
    ...
    <DashboardLayout>
      <div className="p-12 lg:px-16">
        ...
        <MarketplaceFilterBar
          ...
          sort={sort || 'newest'}
          onSortChange={(val: string) => updateSearch({ sort: val as 'newest' | 'popular' | 'alphabetical' })}
