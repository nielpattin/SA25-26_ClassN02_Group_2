...
function BoardComponent() {
  ...
  const { data: allCards = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.cards.board({ boardId }).enriched.get()
      if (error) throw error
      return (data || []).sort((a, b) => a.order - b.order) as Card[]
    },
  })
  ...
  const updateCard = useMutation({
    mutationFn: async ({ id, columnId, order }: { id: string; columnId: string; order: number }) => {
      const { data, error } = await api.cards({ id }).patch({ columnId, order })
      if (error) throw error
      return data
    },
  })
  ...
  const updateColumn = useMutation({
    mutationFn: async ({ id, order }: { id: string; order: number }) => {
      const { data, error } = await api.columns({ id }).patch({ order })
      if (error) throw error
      return data
    },
  })
  ...
  const handleCardDrop = useCallback((sourceData: CardDragData, targetData: Record<string, unknown>) => {
    ...
    if (targetIndex === 0) {
      ...
      newOrder = targetCards.length > 0 ? targetCards[targetCards.length - 1].order + 1 : 1
    } else {
      const prevCard = targetCards[targetIndex - 1]
      const nextCard = targetCards[targetIndex]
  ...
  }, [cardsByColumn, boardId, queryClient, updateCard])
  ...
  const handleColumnDrop = useCallback((sourceData: ColumnDragData, targetData: ColumnDragData) => {
    ...
    let newOrder: number
    if (newIndex === 0) {
      newOrder = targetColumns.length > 0 ? targetColumns[0].order - 1 : 1
    } else if (newIndex >= targetColumns.length) {
      newOrder = targetColumns.length > 0 ? targetColumns[targetColumns.length - 1].order + 1 : 1
    } else {
      const prevCol = targetColumns[newIndex - 1]
      const nextCol = targetColumns[newIndex]
  ...
  }, [columns, boardId, queryClient, updateColumn])
